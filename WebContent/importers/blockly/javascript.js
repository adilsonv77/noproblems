...
Blockly.JavaScript.importer = function(node, parent, options) {
  ...
  switch (node.type) {
    ...
    case "CallExpression":
    	if (node.callee.name == "print") {
    		...
    		var param = this.convert_atomic(node.arguments[0], node, options);
    		this.appendValueInput(block, 'TEXT', param);
    	} else {
    		if (node.callee.name == "read") {
    	        var block = goog.dom.createDom('block');
    	        block.setAttribute('type', 'procedures_callreturn');
    	        var mutation = goog.dom.createDom('mutation');
    	        mutation.setAttribute('name', 'read');
    	        block.appendChild(mutation);
    		}
    	}
    	break;
 /*
    case "CallExpression":
        ...
        var mutation = goog.dom.createDom('mutation');
        mutation.setAttribute('name', node.callee.name);
        block.appendChild(mutation);
        
        for (var j = 0; j < node.arguments.length; j++) {
          var arg = goog.dom.createDom('arg');
          arg.setAttribute('name', node.arguments[j]);
